...
void findPlates(cv::Mat input, std::vector<cv::Point2f> &output){
	...
	Mat edge, cedge, im_contours; //initialize Mats for canny and contours
	vector<vector<Point>> foundContours;	//vector for contours found in canny output
	vector<vector<Point>> foundConvexHulls;	//vector for convex hulls of contours found in canny output
	vector<vector<Point>> filteredContours;	//vector for contours filtered by approximation
	vector<Vec4i> hierarchy;	//vector for hierarchy information output by findContours
	...
	//show edge map generated by canny if in debug mode
	if(debug){
		cedge = Scalar::all(0);
	...
	for(unsigned int i = 0; i < foundContours.size(); i++){
		convexHull(foundContours[i], foundConvexHulls[i]); //generate convex hull for every found contour and store in foundConvexHulls
	}

	if(foundConvexHulls.size()>0){

		//variables used during area comparison
		int area = 0;	//area of current hull
		int max_area=0;	//maximum area found after comparison
		int threshold_approximation=3;	//initial value for threshold in approxPolyDP(..)
		vector<vector<Point>> largest_contour; //storage for largest contour
		vector<accDistanceAndPoint> sortedPoints; //storage for Points of final contour with corresponding distance to origin

		//adjust vector sizes and clone image
		largest_contour.resize(1);
		filteredContours.resize(foundConvexHulls.size());
		im_contours=input.clone();


		//get largest contour by simplification of each contour and successive comparison of area sizes
		for( size_t k = 0; k < foundConvexHulls.size(); k++ ){
				//simplifies polygon (found convex hull) by reducing the number of corner points
				// using an approximation algorithm (for details pls. see: [http://docs.opencv.org/2.4/modules/imgproc/doc/structural_analysis_and_shape_descriptors.html#approxpolydp])
			...
			area = contourArea(filteredContours[k]);
				//compare with largest area found so far, if larger, set new value for max_area and store corresponding contour in largest_contour[]
			if(area>=max_area){
				max_area =  area;
		...
		//simplifying contour until polygon has only 4 corners by incrementing threshold for approximation to ensure correct function of subsequent algorithms
		...
		//storing points of final contour and the accumulated distance to 0 (sum of x and y) in sortedPoints
		for(Point pt : largest_contour[0]){
		...
		//reposition element as order should be for further processing: top_left, top_right, bottom_right, bottom_left
